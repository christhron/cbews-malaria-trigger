...
| `make_paper_figures.py` | Post-processes the cached grid into the exact figures used in the paper. |
| `trigger_grids_10yr_fine.pkl` (+ `_10yr.pkl`, `_exploratory.pkl`) | Cached sweep results (small — a few MB) so figures can be regenerated without re-running the simulation. |
...
`trigger_grids_10yr_fine.pkl` is already checked in.
